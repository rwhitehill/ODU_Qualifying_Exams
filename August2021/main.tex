...
(a) If one measures the total spin along the $z$-axis, one may obtain (sans units of $\hbar$) $1$ and $0$ with probabilities 
\begin{align}
    \eqbox{ P(1) = 1/4, \quad P(0) = 3/4 }
.\end{align}
...
.\end{align}
From this, it is clear that one may obtain $1$ and $0$ for $s$ upon measurement of $\vb*{S}^2$ with probabilities 
\begin{align}    
    \eqbox{ P(1) = 5/8, \quad P(0) = 3/8 }
.\end{align}
...
.\end{align}
The probability to measure $E_{n}$ is then
\begin{align}
\eqbox{
    P(E_{n}) = \begin{cases}
        \frac{96}{n^4 \pi^4} & n = {\rm odd} \\
        0 & n = {\rm even} \\
    .\end{cases}
}
\end{align}
...
(b) From the above, we also can find
\begin{align}
    \eqbox{ \expval{E} = \sum_{n=\rm odd} P(E_{n}) E_{n} = \frac{96}{\pi^{4}} E_1 \sum_{n=\rm odd} \frac{1}{n^2} = \frac{12}{\pi^2} E_{1} }
.\end{align}
...
\begin{align}
    \dv{\expval{A}}{t} = \frac{i}{\hbar} \expval{[H,A]} + \expval{\pdv{A}{t}}
.\end{align}
If our operator does not depend explicitly on time, then the second term is zero.
Thus, we can determine the time dependence of $L_{z}$ as follows.
First, we evalute the commutator
\begin{align}
    [H,L_{z}] = \frac{1}{2m} \Big\{ [p_{x}^2,L_{z}] + [p_{y}^2,L_{z}] + [p_{z}^2,L_z] \Big\} + \lambda [x,L_z] = -i \hbar \lambda y
.\end{align}
This gives us that
\begin{align}
    \dv{\expval{L_z}}{t} = \lambda \expval{y}
.\end{align}
Next, we evaluate
\begin{align}
    [H,y] = \frac{1}{2m} [ p_{y}^2, y ] = - \frac{i \hbar}{m} p_{y}
,\end{align}
which gives
\begin{align}
    \dv{\expval{y}}{t} = \frac{\expval{p_{y}}}{m} = \frac{c}{m}
.\end{align}
where we have used that $[H,p_{y}] = 0$ to establish that $\expval{p_{y}}$ is independent of time.
Unravelling our work, we find
\begin{align}
    \dv{\expval{L_{z}}}{t} = \frac{\lambda c}{m} t + \lambda b \Rightarrow \eqbox{ \expval{L_{z}} = \frac{\lambda c}{2 m} t^2 + \lambda b t + c }
.\end{align}
...
(a) The Hamiltonian above describes a diatomic molecule bound together by a ``spring'' (i.e. they form a harmonic oscillator).


(b) Observe that
\begin{align}
    \pdv{\psi(x_1,x_2)}{x_1} &= \Big( \frac{iP}{2 \hbar} - \frac{x_1 - x_2}{\sigma^2} \Big) \psi(x_1,x_2) \nonumber \\
    \pdv[2]{\psi(x_1,x_2)}{x_1} &= -\frac{1}{\sigma^2} \psi(x_1,x_2) + \Big( \frac{i P}{2 \hbar} - \frac{x_1 - x_2}{\sigma^2} \Big)^2 \psi(x_1,x_2)
.\end{align}
Thus, 
\begin{align}
    \pdv[2]{\psi(x_1,x_2)}{x_1} + \pdv[2]{\psi(x_1,x_2)}{x_2} &= -\frac{1}{\sigma^2} \psi(x_1,x_2) + \Big( \frac{i P}{2 \hbar} - \frac{x_1 - x_2}{\sigma^2} \Big)^2 \psi(x_1,x_2) \nonumber \\
                                                              &\phantom{=} - \frac{1}{\sigma^2} \psi(x_1,x_2) + \Big( \frac{iP}{2 \hbar} + \frac{x_1 - x_2}{\sigma^2} \Big)^2 \psi(x_1,x_2) \nonumber \\
                                                              &= \Bigg[ -\frac{2}{\sigma^2} - \frac{P^2}{2 \hbar^2} + \frac{2(x_1 - x_2)^2}{\sigma^2} \Bigg] \psi(x_1,x2)
.\end{align}
The Hamiltonian acting on $\psi$ then takes the form
\begin{align}
    \eqbox{ \mathcal{H} \psi(x_1,x_2) = \Bigg[ \frac{\hbar^2}{m \sigma^2} + \frac{P^2}{4 m} - \frac{\hbar^2 (x_1 - x_2)^2}{m \sigma^2} + \frac{m \omega^2 (x_1 - x_2)^2}{4} \Bigg] \psi(x_1,x_2) }
.\end{align}
We then see that $\psi(x_1,x_2)$ is a solution to the time-independent Schr\"{o}dinger equation if $\sigma$ is chosen properly.


(c) If we have $\sigma^2 = 4 \hbar^2/(m^2 \omega^2)$, then
\begin{align}
    \eqbox{ E = \frac{m \omega^2}{4} + \frac{P^2}{4 m} }
.\end{align}
...
Our Hamiltonian
\begin{align}
    H = - \vb*{\mu} \cdot \vb*{B} = - \gamma ( \sigma_{x} B_{x} + \sigma_z B_z  ) =
    - \omega \begin{pmatrix} 
        B_{z} & B_{x} \\
        B_{x} & -B_{z}
    \end{pmatrix}
,\end{align}
where $\gamma = g_{e} e \hbar / (4mc)$.
...
\begin{align}
    \begin{vmatrix}
        ...
        B_{x} & -(B_{z} + \lambda)
    \end{vmatrix}
    = \lambda^2 - ( B_{x}^2 + B_{z}^2 ) = 0 \Rightarrow \lambda = \pm \sqrt{B_{x}^2 + B_{z}^2} = \pm B
.\end{align}
...
\begin{align}
    ...
    \end{pmatrix}
    = 0
    \Rightarrow \chi_{\pm} = a_{1,\pm} \begin{pmatrix}
        1 \\
        (B_{z} \mp B) / B_{x}
    \end{pmatrix}
.\end{align}
Finally, we normalize our eigenvectors:
\begin{align}
    |a_{1,\pm}|^2 \Bigg[ 1 + \frac{B_{z}^2 \mp 2 B_{z} B + B^2}{B_{x}^2} \Bigg] = 1 \Rightarrow \chi_{\pm} = \frac{1}{\sqrt{2B(B \mp B_{z})}} \begin{pmatrix}
    B_x \\ B_{z} \mp B
    \end{pmatrix}
.\end{align}

From the problem statement, our particle has state
\begin{align}
